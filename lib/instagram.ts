...
// Use Authorization header instead of URL query param to prevent token leakage in logs
...
export async function getRecentPosts(limit = 10): Promise<InstagramPost[]> {
  ...
  const res = await fetch(
    `${IG_API}/${id}/media?fields=${encodeURIComponent(fields)}&limit=${Math.min(limit, 50)}`,
...
export async function getInsights(mediaId: string): Promise<Record<string, number>> {
  ...
  const res = await fetch(
    `${IG_API}/${mediaId}/insights?metric=${encodeURIComponent(metrics)}`,
    { headers: authHeaders(token) }
  );
  if (!res.ok) return {};
  const data = await res.json();
  return Object.fromEntries(
    (data.data ?? []).map((m: { name: string; values: Array<{ value: number }> }) => [
      m.name,
      m.values[0]?.value ?? 0,
    ])
  );
...
// Send DM reply via Facebook Graph API (Instagram DM uses FB API endpoint)
export async function sendDmReply(recipientId: string, text: string): Promise<void> {
  ...
  if (!res.ok) throw new Error("Failed to send Instagram DM reply");
}
